...
def generate_stats_svg(data):
    ...
    for idx, (label, val, path) in enumerate(stats_items):
        ...
        <g class="stat-row" style="animation-delay: {anim_delay}ms;">
            ...
            <text x="180" y="{y}" class="value">{val:,}</text>
        </g>
        """)
    ...
    <text x="340" y="166" class="label" font-size="10">Following: <tspan class="value" font-size="10">{data.get('following', {}).get('totalCount', 0)}</tspan></text>
    """)
